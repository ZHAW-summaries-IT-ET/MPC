...
\begin{example2}{Ex.\ S12/13 -- CB1 / Jetson terminology}\\
...
\begin{KR}{Recipe: Task-Decompose an Algorithm}\\
    ...
    \paragraph{State parallelism + joins}
\end{KR}
...
\begin{KR}{Recipe: Dependency Analysis}\\
    \paragraph{Group} same data/constraints together.
    \paragraph{Order} add constraint per producer$\to$consumer (+ ghost-exchange before boundary).
    \paragraph{Classify} read-only / local / read-write; strict ordering removes ``potentially''.
\end{KR}
