...
\begin{KR}{Recipe: Decode a NEON Intrinsic}
    ...
    \textbf{op} the operation (\texttt{mul}, \texttt{mla}, \ldots).
    \textbf{q?} present $\Rightarrow$ 128-bit Q, else 64-bit D.
    \textbf{type} \texttt{<base><bits>x<count>}: \emph{count} = lanes, \emph{bits} = lane width.
    \textbf{result} lane-wise op over \emph{count} elements.
\end{KR}
