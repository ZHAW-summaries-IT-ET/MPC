...
\begin{concept}{Lernziele (Learning Goals)}
    \begin{enumerate}
        \item Know and explain the \textbf{terms} (host/device/CU/PE/FU/UE; dual meaning of ``task'') [S12]
        \item Know and explain the \textbf{general solutions} Algorithm-Structure patterns (organise by task vs data) [S22/25]
        \item Trace the \textbf{top-down path} through the four pattern layers (Sobel) [S23]
        \item Know how to apply the \textbf{task-decomposition} pattern (functional vs loop splitting) [S26]
        \item Explain the difference between a \textbf{CFG and a call graph} [S29]
        \item Explain the \textbf{task-parallelism dependency} terms (embarrassingly parallel; order; removable/separable) [S35]
        \item Explain the \textbf{scheduling} terms (static, dynamic, task queue, work-stealing) [S36]
    \end{enumerate}
\end{concept}

\subsection{LZ1 Vocabulary \& Platform Model}
...
\begin{example2}{Ex.\ LZ1 CB1 / Jetson terminology}
...
\subsection{LZ2 General Solutions (Algorithm Structure)}
...
\begin{example2}{Ex.\ LZ2 Pick the pattern}
...
\subsection{LZ3 Top-Down Pattern Path}
...
\begin{example2}{Ex.\ LZ3 Trace Sobel through the layers}
...
\subsection{LZ4 Task Decomposition}
...
\begin{example2}{Ex.\ LZ4 Decompose Sobel}
...
\subsection{LZ5 CFG vs Call Graph}
...
\begin{example2}{Ex.\ LZ5 Which graph?}
...
\subsection{LZ6 Task-Parallelism Dependencies}
...
\begin{example2}{Ex.\ LZ6 Classify the dependencies}
...
\subsection{LZ7 Scheduling (Task $\to$ UE)}
...
\begin{example2}{Ex.\ LZ7 Pick a schedule}
