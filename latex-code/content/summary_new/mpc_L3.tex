...
\begin{concept}{Lernziele (Learning Goals)}
    \emph{This deck labels no explicit Lernziele these are derived from its ``Session 1'' topic agenda.}
    \begin{enumerate}
        \item Explain \textbf{what OpenMP is} and its execution model (shared memory, Fork-Join + SPMD, implicit barrier) [s4--6]
        \item \textbf{Data scope}: worksharing + variable scope + reduction; predict code behaviour [s7--9]
        \item \textbf{IPC synchronisation}: critical sections (atomic / critical / semaphore / mutex / locks) [s10--15]
        \item \textbf{Thread synchronisation}: barriers (implicit, \texttt{nowait}, \texttt{ordered}) [s16--20]
        \item \textbf{Scheduling}: task management (master/single/sections) + loop schedules [s21--27]
        \item \textbf{Runtime interface}: how the thread count is determined [s28--29]
    \end{enumerate}
\end{concept}
