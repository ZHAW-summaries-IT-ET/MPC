...
\begin{example2}{Ex.\ Classify these}
...
\begin{example2}{Ex.\ SPMD vs SIMD}
...
\subsection{SIMD Vectorisation Motivation}
...
\begin{example2}{Ex.\ Which vectorise?}
...
\subsection{NEON Basics (Vector MAC Unit)}
...
\begin{example2}{Ex.\ Decode \texttt{VQADD.s16}}
...
\begin{example2}{Ex.\ (s19) Decode intrinsics}
...
\begin{example2}{Ex.\ (s20) Lab: vectorise statistics}
...
\begin{example2}{Ex.\ ABS controller}
...
for (int i = 0; i < N; i++)          // first vector
  for (int j = 0; j < N; j++) {      // second vector
    ...
    int32x4_t vp = vmulq_s32(va, vb); // lane-wise mult.
    out[i][j] = vaddvq_s32(vp); 
