...
\begin{concept}{Lernziele (Learning Goals)}
    \begin{enumerate}
        \item Explain \textbf{cache} [s4] and \textbf{paging} [s5]
        \item Explain the \textbf{two types of thrashing} [s13]
        \item Explain the \textbf{cost of cache-oblivious} code (context switch) [s14]
        \item Explain \textbf{cache writeback} [s16] and \textbf{multicore L1 read} on shared L2 [s17]
        \item Explain \textbf{coherence} + conditions [s19] and \textbf{consistency} [s20]
        \item Explain how coherence is \textbf{managed}: snooping [s21] and \textbf{directory} [s22]
        \item Explain \textbf{false sharing} [s23]
        \item Understand the \textbf{memory architectures} UMA/NUMA/SUMA + their limits [s25--29]
    \end{enumerate}
    \emph{Worked-example slides (no explicit Lernziel): the memory-access trace [s6--12] and ASID/CPU-pinning [s24].}
\end{concept}

\subsection{LZ1 Cache \& Paging}
...
\begin{example2}{Ex.\ LZ1 Why is a[1] fast after a[0]?}
...
\subsection{The Memory Access Walk (worked)}
...
\begin{example2}{Ex.\ LZ2 Same-slot loop}
...
\subsection{LZ3 Cost of Cache-Oblivious Code}
...
\begin{example2}{Ex.\ LZ3 CPI after a switch}
...
\subsection{LZ4 Multicore L1/L2 (Read \& Writeback)}
...
\begin{example2}{Ex.\ LZ4 Read then flush}
...
\subsection{LZ5 Coherence \& Consistency}
...
\begin{example2}{Ex.\ LZ5 The read sequences}
...
\subsection{LZ6 Managing Coherence}
...
\begin{example2}{Ex.\ LZ6 P1 writes a shared line}
...
\begin{example2}{Ex.\ LZ7 Adjacent counters}
...
\subsection{LZ8 Memory Architectures}
...
\begin{example2}{Ex.\ LZ8 Limits \& OS support}
