...
\begin{concept}{Lernziele (Learning Goals)}
    \begin{enumerate}
        \item \textbf{Distributed architectures} protocols + common problems [s6]
        \item The \textbf{six fundamental MPI operations} [s7--8]
        \item \textbf{Communication models} (communicator/rank, P2P vs collective) [s9]
        \item The \textbf{run-time} (compile/run, SPMD/MIMD, binding) [s10--13]
        \item \textbf{Blocking vs non-blocking} [s14]
        \item The \textbf{four blocking send modes} [s15--16]
        \item \textbf{Non-blocking} send/recv (Wait/Test) [s17]
        \item \textbf{Message structure} + data types (+ derived) [s18--19]
        \item \textbf{Collectives} (Bcast / Scatter / Gather) [s20]
        \item \textbf{Reduction} (\texttt{MPI\_Reduce}) [s21]
    \end{enumerate}
\end{concept}

\subsection{LZ1 Distributed Architectures}
...
\begin{example2}{Ex.\ LZ1 Struct across machines}
...
\subsection{LZ2 The Six Fundamental Operations}
...
\begin{example2}{Ex.\ LZ2 Name the six}
...
\subsection{LZ3 Communication Models}
...
\begin{example2}{Ex.\ LZ3 Communicator / rank}
...
\begin{example2}{Ex.\ LZ4 Build / run / oversubscribe}
...
\subsection{LZ5 Blocking vs Non-blocking}
...
\begin{example2}{Ex.\ LZ5 Why non-blocking?}
...
\subsection{LZ6 The Four Blocking Send Modes}
...
\begin{example2}{Ex.\ LZ6 Safest vs cheapest}
...
\subsection{LZ7 Non-blocking Send/Recv}
...
\begin{example2}{Ex.\ LZ7 Did it finish?}
...
\subsection{LZ8 Message Structure \& Data Types}
...
\begin{example2}{Ex.\ LZ8 Two parts + a struct}
...
\begin{example2}{Ex.\ LZ9 Distribute then collect}
...
\begin{example2}{Ex.\ LZ10 Combine partial sums}
